...
void test_address_helpers() {
    ...
    sockaddr_in ipv4{};
    expect(unp::net::pton("127.0.0.1", ipv4), "pton parses an IPv4 address");
    expect(ipv4.sin_family == AF_INET, "pton sets the IPv4 family");
    expect(unp::net::ntop(ipv4) == "127.0.0.1", "ntop formats an IPv4 address");

    const unp::net::socket_address_view ipv4_view =
        unp::net::auto_sockaddr(ipv4);
    expect(ipv4_view.address == reinterpret_cast<const sockaddr*>(&ipv4),
           "auto_sockaddr points at the source IPv4 address");
    expect(ipv4_view.length == static_cast<unp::net::socket_length_t>(sizeof(ipv4)),
           "auto_sockaddr reports the IPv4 address size");

    sockaddr_in6 ipv6{};
    expect(unp::net::pton("::1", ipv6), "pton parses an IPv6 address");
    expect(ipv6.sin6_family == AF_INET6, "pton sets the IPv6 family");
    expect(unp::net::ntop(ipv6) == "::1", "ntop formats an IPv6 address");

    sockaddr_in invalid{};
    expect(!unp::net::pton("not-an-address", invalid),
           "pton rejects an invalid address");
    expect(!unp::net::pton(nullptr, invalid),
           "pton rejects a null address string");
}

void test_socket_ownership() {
    static_assert(!std::is_copy_constructible_v<unp::net::sockfd>);
    static_assert(!std::is_copy_assignable_v<unp::net::sockfd>);
    static_assert(std::is_nothrow_move_constructible_v<unp::net::sockfd>);
    static_assert(std::is_nothrow_move_assignable_v<unp::net::sockfd>);

    unp::net::sockfd socket =
        unp::net::sockfd::create(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    expect(socket.valid(), "sockfd::create creates a TCP socket");

    const unp::net::native_socket_t native_socket = socket.get();
    socket.reset(native_socket);
    expect(socket.get() == native_socket,
           "sockfd::reset ignores the currently owned socket");

    unp::net::sockfd moved = std::move(socket);
    expect(!socket.valid(), "moving sockfd clears the source");
    expect(moved.valid(), "moving sockfd preserves the socket");

    const unp::net::native_socket_t released_socket = moved.release();
    expect(!moved.valid(), "sockfd::release clears the owner");

    unp::net::sockfd adopted{released_socket};
...
void test_socket_io() {
    unp::net::sockfd listener = unp::net::sockfd::create(
        AF_INET,
        SOCK_STREAM,
        IPPROTO_TCP
    );
    ...
    expect(unp::net::pton("127.0.0.1", listener_address),
           "pton prepares the loopback listener address");
    listener_address.sin_port = 0;

    const unp::net::socket_address_view listener_view =
        unp::net::auto_sockaddr(listener_address);
    expect(unp::net::bind_socket(listener.get(), listener_view) == 0,
           "bind_socket binds a socket_address_view");
    expect(unp::net::listen_socket(listener.get(), 1) == 0,
           "listen starts the test listener");

    unp::net::socket_length_t listener_length =
        static_cast<unp::net::socket_length_t>(sizeof(listener_address));
    expect(::getsockname(
               listener.get(),
    ...
    unp::net::sockfd client = unp::net::sockfd::create(
        AF_INET,
        SOCK_STREAM,
        IPPROTO_TCP
    );
    expect(client.valid(), "create makes a client socket");
    expect(unp::net::connect_socket(client.get(), listener_view) == 0,
           "client connects to the loopback listener");

    sockaddr_storage peer_address{};
    unp::net::socket_length_t peer_length =
        static_cast<unp::net::socket_length_t>(sizeof(peer_address));
    unp::net::sockfd peer = unp::net::accept_socket(
        listener.get(),
        peer_address,
        peer_length
    );
    ...
    expect(unp::net::io::send_all(
               peer.get(),
               message.data(),
               message.size()
           ),
           "send_all sends the complete message");

    char buffer[64]{};
    expect(unp::net::io::readn(client.get(), nullptr, 0) == 0,
           "readn accepts an empty null buffer");
    expect(unp::net::io::readn(client.get(), nullptr, 1) < 0,
           "readn rejects a non-empty null buffer");

    const unp::net::socket_io_result_t received = unp::net::io::readn(
        client.get(),
        buffer,
        message.size()
    );
    expect(received == static_cast<unp::net::socket_io_result_t>(message.size()),
    ...
    expect(unp::net::io::send_all(peer.get(), lines.data(), lines.size()),
           "send_all sends line-oriented test data");
    peer.reset();

    unp::net::io::buffered_reader reader{client.get()};
    char line_buffer[7]{};
    const unp::net::socket_io_result_t first_chunk =
        unp::net::io::readline(reader, line_buffer);
    expect(first_chunk == 6 && std::string(line_buffer) == "first ",
           "fixed-buffer readline returns a truncated chunk");

    static_assert(std::is_nothrow_move_constructible_v<
        unp::net::io::buffered_reader
    >);
    static_assert(std::is_nothrow_move_assignable_v<
        unp::net::io::buffered_reader
    >);
    unp::net::io::buffered_reader moved_reader{std::move(reader)};
    expect(!reader.valid() && moved_reader.valid(),
           "moving a buffered reader transfers its socket binding");

    std::string line;
    const unp::net::socket_io_result_t first_line_end =
        unp::net::io::readline(moved_reader, line);
    expect(first_line_end == 5 && line == "line\n",
           "move construction preserves unread buffered data");

    unp::net::io::buffered_reader assigned_reader;
    ...
    const unp::net::socket_io_result_t second_chunk =
        unp::net::io::readline(assigned_reader, line, 3);
    expect(second_chunk == 3 && line == "sec",
           "move assignment preserves unread buffered data");

    const unp::net::socket_io_result_t final_chunk =
        unp::net::io::readline(assigned_reader, line);
    expect(final_chunk == 3 && line == "ond",
           "string readline returns partial data before EOF");
    expect(unp::net::io::readline(assigned_reader, line) == 0 && line.empty(),
           "string readline reports EOF with no remaining data");

    assigned_reader.reset(unp::net::invalid_socket);
...
int main() {
    unp::net::socket_env environment;
